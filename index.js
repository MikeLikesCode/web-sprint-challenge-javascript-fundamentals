...
/* The nestedFunction can access the variable internal because its inside of the parent scope and thats defined
as its lexical enviorment. The parent function cannot reach into the nestedFunction because its lexical enviorment
is deifined outside of it. */
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const strings = [];
    arr.forEach((item) => {
      strings.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`);
    })
  ...
  */
  ...
  function USApop(arr){
    return arr.reduce((acc,index) => acc + index.population , 0);
  }
